...
import { ProjectsDisplay, UserProjectCard } from "../components";
...
const User = () => {
  ...
  const { aoeth: aoethBalance, refresh: refreshAoethBalance } =
    useUserAoETH(address);
  useEffect(() => {
  ...
  const totalStaked = stakedAmounts
    ? Object.values(stakedAmounts).reduce(
        (acc: number, curr) => acc + curr.aoeth,
        0
      )
    : null;
